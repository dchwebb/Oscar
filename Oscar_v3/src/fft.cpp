...
void FFT::Activate()
{
	...
	samples = ui.cfg.displayMode == DispMode::Fourier ? fftSamples : waterfallSamples;
}
...
void FFT::PopulateOverlayBuffer(const float* sampleBuffer)
{
	...
	for (uint16_t p = 0; p < fftSamples - lcd.drawWidth; p++) {
		if (trigger > osc.cfg.triggerY && (4 * (2047 - sampleBuffer[p])) < osc.cfg.triggerY) {
			start = p;
			break;
		}
...
void FFT::DisplayFFT(const float* sinBuffer)
{
	...
	const uint16_t overlayColour = cfg.channel == channelA ? RGBColour::DullGreen : cfg.channel == channelB ? RGBColour::DullBlue : RGBColour::DullOrange;

	// Cycle through each column in the display and draw
	for (uint16_t i = 1; i <= lcd.drawWidth; i++) {
		uint16_t harmColour = RGBColour::Blue;
		...
		for (uint32_t h = 0; h <= lcd.drawHeight; ++h) {
			...
			} else if (cfg.traceOverlay && h >= AY.first && h <= AY.second) {		// Draw oscilloscope trace as overlay
